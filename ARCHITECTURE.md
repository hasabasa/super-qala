...
**Команда:** backend — основатель + Claude Code. Frontend (Flutter) — наёмный разработчик. Дизайн — по мере необходимости.
...
| Frontend пишет посторонний человек | **API — это контракт.** OpenAPI-схема генерируется автоматически и является единственным источником правды. Разработчик не должен спрашивать «а что тут возвращается» |
...
| **Мобильное приложение** | Flutter | Один разработчик закрывает iOS и Android. Сильный рынок найма в СНГ |
